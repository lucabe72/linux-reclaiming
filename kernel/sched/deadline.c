...
static void sub_running_bw(struct sched_dl_entity *dl_se, struct dl_rq *dl_rq)
{
	...
	if (rq->rd->running_bw < 0)
		rq->rd->running_bw = 0;
}

static void sub_rq_bw(struct sched_dl_entity *dl_se, struct dl_rq *dl_rq)
{
}
...
void init_dl_rq(struct dl_rq *dl_rq)
{
	...
	if (global_rt_runtime() == RUNTIME_INF)
		...
		dl_rq->non_deadline_bw = (1 << 20) - to_ratio(global_rt_period(), global_rt_runtime());
#endif
}
...
u64 grub_reclaim(u64 delta, struct rq *rq, u64 u)
{
	...
	delta_exec2 = div64_long(delta *
		((m << 20) - rd->dl_bw.total_bw + rd->running_bw), m) >> 20;
	if (delta_exec1 > delta_exec2)
		return delta_exec2;
...
static enum hrtimer_restart inactive_task_timer(struct hrtimer *timer)
{
	...
	if (dl_se->dl_new) {
		printk(KERN_ERR "Problem! New task was inactive?\n");
		goto unlock;
	}
...
static void enqueue_task_dl(struct rq *rq, struct task_struct *p, int flags)
{
	...
	if (p->dl.dl_throttled && !(flags & ENQUEUE_REPLENISH)) {
		if (hrtimer_try_to_cancel(&p->dl.inactive_timer) < 0) {
			printk(KERN_WARNING "Waking up a depleted task, but cannot cancel inactive timer!\n");
...
static void switched_from_dl(struct rq *rq, struct task_struct *p)
{
	 ...
	 */
	if (!hrtimer_callback_running(&p->dl.inactive_timer))
		sub_running_bw(&p->dl, &rq->dl);
